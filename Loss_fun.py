...
def make_loss(num_classes, camera_num=0, use_gpu=True):
    center_criterion = CombinedCenterLoss(
        num_classes=num_classes,
        use_gpu=use_gpu
    )
    ...
    def loss_func(score, feat, target, cam_logits=None, target_cam=None):
        ...
        if isinstance(score, list):
            id_loss_local = sum(xent(scor, target) for scor in score[1:]) / len(score[1:])
        ...
        # Triplet loss
        if isinstance(feat, list):
            tri_loss_local = sum(triplet(fea, target)[0] for fea in feat[1:]) / len(feat[1:])
